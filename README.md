...
- **外部媒體管理器模式**：選 Jellyfin / Emby 或 Kodi，刮削後自動生成對應命名的 poster + fanart 與相容 NFO，掛進媒體庫即正確顯示（poster 與 NFO 三者通用；`{stem}-fanart` 僅 Jellyfin／Kodi 讀取，Emby 不認此 fanart 命名、背景圖不顯示）。

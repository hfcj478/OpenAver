...
class DesktopLifecycle:
    ...
    def __init__(
        ...
        self.prompt = prompt or show_close_dialog
        self.tray = None
...
def _show_task_dialog() -> CloseDecision:
    ...
    buttons = (TaskDialogButton * 3)(
        TaskDialogButton(CMD_CLOSE_TRAY, "最小化到系統匣\nOpenAver 繼續在後台運行"),
        TaskDialogButton(CMD_CLOSE_EXIT, "退出 OpenAver\n同時停止後台服務"),
        TaskDialogButton(2, "取消"),
    )
    config = TaskDialogConfig()
    config.cbSize = ctypes.sizeof(TaskDialogConfig)
    config.dwFlags = 0x0008 | 0x0010  # TDF_ALLOW_DIALOG_CANCELLATION | TDF_USE_COMMAND_LINKS
    config.pszWindowTitle = "OpenAver"
    config.pszMainInstruction = "要退出 OpenAver，還是讓它在後台運行？"
    config.pszContent = "最小化到系統匣後，可雙擊右下角圖示重新開啟。"
    config.cButtons = len(buttons)
    config.pButtons = buttons
    config.nDefaultButton = CMD_CLOSE_TRAY
    config.pszVerificationText = "記住我的選擇"

    selected = ctypes.c_int(2)
    ...
    result = task_dialog(ctypes.byref(config), ctypes.byref(selected), ctypes.byref(radio), ctypes.byref(verified))
    if result != 0:
        raise OSError(result, "TaskDialogIndirect failed")
    return _map_dialog_result(selected.value, bool(verified.value))


def _show_message_box() -> CloseDecision:
    # Yes=exit, No=tray, Cancel=cancel. The fallback cannot persist a choice.
    result = ctypes.windll.user32.MessageBoxW(
        None,
        "選擇「是」退出程式，選擇「否」最小化到系統匣，或選擇「取消」返回。",
        "關閉 OpenAver",
...
def show_tray_unavailable() -> None:
    if sys.platform != "win32":
        return
    try:
        ctypes.windll.user32.MessageBoxW(
            None,
            "系統匣圖示無法啟動，OpenAver 將保持開啟。請從視窗選擇退出。",
            "OpenAver",
...
class NativeTrayIcon:
    ...
    def _run_windows(self) -> None:
        ...
        def show_menu() -> None:
            ...
            user32.AppendMenuW(menu, 0x0000, CMD_OPEN, "開啟 OpenAver")
            ...
            for command, value, label in (
                (CMD_CLOSE_ASK, CLOSE_ASK, "每次關閉時詢問"),
                (CMD_CLOSE_TRAY, CLOSE_TRAY, "最小化到系統匣"),
                (CMD_CLOSE_EXIT, CLOSE_EXIT, "退出程式"),
            ):
                flags = 0x0000 | (0x0008 if action == value else 0)
                user32.AppendMenuW(submenu, flags, command, label)
            user32.AppendMenuW(menu, 0x0010, submenu, "關閉按鈕行為")
            user32.AppendMenuW(menu, 0x0800, 0, None)
            user32.AppendMenuW(menu, 0x0000, CMD_QUIT, "退出 OpenAver")
        ...
        def wnd_proc(hwnd, message, w_param, l_param):
            ...
            if message == wm_tray:
                ...
                if event in {0x0203, 0x0400}:  # WM_LBUTTONDBLCLK, NIN_SELECT
                    dispatch(CMD_OPEN)

...
def emit_notification(
    ...
    with _lock:
        ...
        # 去重**只管定時整理自己**。原始動機就只在這裡（每 12 小時重演一次的
        # 「定時整理失敗」不該塞滿抽屜），但這個函式是全站共用的：不收窄的話，
        # 從來沒開過定時整理的人也會受影響——連按兩次「產生影片列表」而中間沒開過
        # 通知抽屜時，第二次的「掃描開始」會被吃掉，使用者以為掃描沒跑
        # （main 上沒有去重，兩則都會出現）。
        if task_type == "auto_organize" and any(
